...
When posting data from this form, EXT:fluidbackend has multiple modes of operation:
...
Much of it is all up to you. EXT:fluidbackend provides only the rendering and controller affinity (and the mentioned output
storage methods), the rest is to be achived using custom controllers and as such, you are only limited by your own creativity with
the highly dynamic Flux forms which make up the form that results in controller input...

Each type of target (JSON, TS, etc) is configured by inserting a ViewHelper (from EXT:fluidbackend) into the `<flux:flexform>`
tag - and add any number of additional tags to configure additional data writing targets.
